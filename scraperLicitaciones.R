## Scraper de licitaciones para 2015-2017
...
licitaciones <- paste0("http://www.madrid.org/cs/Satellite?c=Page&cid=1224915242285&codigo=PCON_&fechaFormalizacionDesde=01%2F01%2F2015&fechaFormalizacionHasta=31%2F12%2F2017&idPagina=1224915242285&language=es&newPagina=", paginas,"&numPagListado=5&pagename=PortalContratacion%2FComunes%2FPresentacion%2FPCON_resultadoBuscadorAvanzado&paginaActual=1&paginasTotal=249&rootelement=PortalContratacion%2FComunes%2FPresentacion%2FPCON_resultadoBuscadorAvanzado&site=PortalContratacion&tipoPublicacion=Convocatoria+anunciada+a+licitaci%C3%B3n")
...
write_csv(contratos, path = "data/licitaciones_links.csv", append = FALSE, col_names = FALSE)

p <- length(licitaciones)
...
for (licitacion in licitaciones) {
  ...
  write_csv(contratos, path = "data/licitaciones_links.csv", append = TRUE, col_names = FALSE)
  print(p)
  p <- p - 1
}

df <- read_csv("data/licitaciones_links.csv")
licitaciones.15.17 <-  df$links

## scrapeamos las licitaciones con publicidad
...
write_tsv(line, path = "data/prueba_licitaciones.tsv", append = FALSE, col_names = FALSE)

for (url in 1:length(licitaciones.15.17)) {
  x <- GET(licitaciones.15.17[url], add_headers('user-agent' = desktop_agents[sample(1:10, 1)]))
  ...
  line <- data.frame(estado, tipo_publicacion, objeto, n_expediente, referencia, tipo_contrato, adjudicadora, procedimiento, sin.impuestos, importe.total, fecha_publicacion, fecha_formalizacion, fecha_formalizacion_BOCM, duracion, adjudicatario, NIF, conIVA, sinIVA, licitaciones.15.17[url])
  print(line)
  
  try(write_tsv(line, path = "data/prueba_licitaciones.tsv", append = TRUE, col_names = FALSE))
}
