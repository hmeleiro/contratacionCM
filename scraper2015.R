...
menores2015 <- paste0("http://www.madrid.org/cs/Satellite?c=Page&cid=1224915242285&codigo=PCON_&fechaFormalizacionDesde=01%2F01%2F2015&fechaFormalizacionHasta=31%2F12%2F2015&idPagina=1224915242285&language=es&newPagina=", paginas,"&numPagListado=5&pagename=PortalContratacion%2FComunes%2FPresentacion%2FPCON_resultadoBuscadorAvanzado&paginaActual=1&paginasTotal=49382&procedimientoAdjudicacion=Contratos+Menores&rootelement=PortalContratacion%2FComunes%2FPresentacion%2FPCON_resultadoBuscadorAvanzado&site=PortalContratacion&tipoPublicacion=Contratos+Menores")

contratos_tot.2015 <- NA

contratos <- data.frame("links")
write_csv(contratos, path = "data/links2015.csv", append = FALSE, col_names = FALSE)

p <- length(menores2015)

for (menor in menores2015) {
  ...
  write_csv(contratos, path = "data/links2015.csv", append = TRUE, col_names = FALSE)
  print(p)
  p <- p - 1
}

df <- read_csv("data/links2015.csv")
contratos_tot.2015 <-  df$links
...
write_tsv(line, path = "data/prueba_2015.tsv", append = FALSE, col_names = FALSE)


for (url in contratos_tot.2015) {
  ...
  write_tsv(line, path = "data/prueba_2015.tsv", append = TRUE, col_names = FALSE)
}
